...
import com.oa7.client.TencentMapClient;
...
/**
 * 地址工具类 —— 对外仍暴露静态方法，
 * 内部通过 Spring 注入的 TencentMapClient 完成调用。
 */
@Component
@Slf4j
public class LocationUtil {
    ...
    /** 静态字段注入小技巧 */
    private static TencentMapClient tencentMapClient;

    @Autowired
    public void setTencentMapClient(TencentMapClient client) {
        LocationUtil.tencentMapClient = client;
    }
    ...
    public static String getAddressFromCoordinates(String coordinates) {
        ...
        try {
            ...
            if (resp.ok() && resp.result != null) {
                ...
                return "位置解析失败：" + resp.message;
            }
        } catch (Exception e) {
            return "位置解析异常：" + e.getMessage();
        }
    }
